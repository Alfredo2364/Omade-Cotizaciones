...
<style>
    ...
    .quote-form-card {
        background: var(--surface-2); width: 100%; max-width: 650px;
    ...
    /* Header */
    .form-header {
        padding: 20px 30px; background: var(--surface-2); border-bottom: 1px solid var(--border);
        display: flex; justify-content: space-between; align-items: center;
    }
    .header-title { display: flex; align-items: center; gap: 15px; }
    .icon-box {
        width: 40px; height: 40px; background: rgba(59, 130, 246, 0.1); color: #3b82f6;
        border-radius: 12px; display: flex; align-items: center; justify-content: center;
        font-size: 1.2rem;
    }
    .header-title h3 { margin: 0; font-size: 1.25rem; color: var(--text-color); font-weight: 700; }
    ...
    .section-title {
        ...
        color: var(--muted); letter-spacing: 0.5px; margin-bottom: 15px;
    }
    ...
    .form-group label {
        display: block; margin-bottom: 8px; font-weight: 600; color: var(--muted); font-size: 0.9rem;
    }
    ...
    .input-wrapper input, .input-wrapper select, textarea {
        ...
        border-radius: 12px; font-size: 0.95rem; color: var(--text-color);
    ...
    .input-wrapper input:focus, .input-wrapper select:focus, textarea:focus {
        background: var(--surface-2); border-color: #3b82f6; box-shadow: 0 0 0 4px rgba(59, 130, 246, 0.1);
    }

    /* Footer */
    .form-footer {
        padding: 20px 30px; background: var(--surface-3); border-top: 1px solid var(--border);
        display: flex; justify-content: flex-end; gap: 15px;
    }
    
    .btn-cancel {
        padding: 12px 24px; background: var(--surface-2); border: 1px solid var(--border);
        color: var(--text-muted); font-weight: 600; border-radius: 10px; cursor: pointer;
        transition: 0.2s;
    }
...
<div class="card" style="padding: 0; overflow: hidden; border: none; box-shadow: var(--card-shadow);">
     <div class="card-header-styled" style="background: var(--surface-3); padding: 15px 20px; border-bottom: 1px solid var(--border);">
        <h3 style="margin: 0; color: var(--text-color);">Listado de Solicitudes</h3>
    </div>
    <div class="table-container">
        <table class="table-premium">
            ...
            <tbody>
                ...
                if (empty($quotes)) {
                    echo "<tr><td colspan='7' style='text-align:center; padding: 30px; color: var(--muted);'>No hay cotizaciones pendientes.</td></tr>";
                }

                foreach ($quotes as $q): ?>
                <tr>
                    <td style="font-weight: 600; color: var(--muted);">#<?= $q['id'] ?></td>
                    <td>
                        <div style="font-weight: 600; color: var(--text-color);"><?= htmlspecialchars($q['client_name']) ?></div>
                    </td>
                    <td style="color: var(--muted); font-size: 0.9rem;"><?= htmlspecialchars($q['client_email']) ?></td>
                    <td style="font-size: 0.85rem; color: var(--muted);"><?= date('M d, Y', strtotime($q['created_at'])) ?></td>
                    <td>
                        <?php if($q['status'] == 'pending'): ?>
                            <span class="badge" style="background: var(--surface-3); color: var(--muted); border: 1px solid var(--border);"><i class="far fa-clock"></i> Pendiente</span>
    ...
    <style>
        ...
        .table-premium td { padding: 12px 15px; border-bottom: 1px solid var(--border); vertical-align: middle; color: var(--text-color); }
...
<div class="card" id="details-card" style="margin-top: 30px; border-top: 4px solid #3b82f6;">
    ...
    <div style="background: var(--surface-3); padding: 20px; border-radius: 12px; margin: 15px 0; border: 1px solid var(--border);">
        ...
        <div style="margin-top: 15px; border-top: 1px solid var(--border); padding-top: 10px;">
            ...
            <p style="color: var(--text-color); line-height: 1.5;"><?= nl2br(htmlspecialchars($selected_quote['description'])) ?></p>
        </div>
    </div>
    ...
    <form method="POST">
        ...
        <div style="margin-top: 25px;">
            <h4 style="margin: 0 0 15px 0; color: var(--text-color); font-size: 1.1rem;">Asignar Productos (Cotización)</h4>
            ...
            <div style="position: relative; margin-bottom: 20px; z-index: 100;">
                <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                    ...
                    <div style="flex: 1; display: flex; gap: 8px; min-width: 280px; align-items: center;">
                        <input type="text" id="custom-name" placeholder="Item Manual" style="flex: 1; padding: 10px; border: 1px solid var(--border); border-radius: 8px; min-width: 0; background: var(--surface-3); color: var(--text-color);">
                        <input type="number" id="custom-price" placeholder="$" style="width: 90px; padding: 10px; border: 1px solid var(--border); border-radius: 8px; flex-shrink: 0; background: var(--surface-3); color: var(--text-color);">
            ...
            <div class="table-container">
                <table class="table" style="font-size: 0.95rem; width: 100%; min-width: 600px;">
                    <thead style="background: var(--surface-3); color: var(--muted);">
                        <tr>
        ...
        <style>
            ...
            .search-results-dropdown {
                ...
                box-shadow: var(--card-shadow); z-index: 1000;
                            ...
                            products.slice(0, 10).forEach(p => {
                                ...
                                div.innerHTML = `
                                    <div>
                                        ...
                                        <small style="color: var(--muted);">${p.code || p.product_code || 'S/C'}</small>
                                    </div>
                                    <div style="font-weight: 700; color: var(--secondary-color);">$${parseFloat(p.price).toFixed(2)}</div>
                                `;
                                div.onclick = () => {
                                    addItemToQuote(p);
                                    searchInput.value = '';
            ...
            function renderQuoteItems() {
                ...
                attachedItems.forEach((item, index) => {
                    ...
                    tr.innerHTML = `
                        ...
                        <td style="padding: 10px; text-align: right; font-weight: bold; color: var(--text-color);">$${rowTotal.toFixed(2)}</td>
                        <td style="padding: 10px; text-align: center;">
        ...
        <div style="margin-top: 20px; display: flex; gap: 20px; flex-wrap: wrap;">
            <div style="flex: 1; min-width: 200px;">
                ...
                <input type="number" step="0.01" name="quote_discount" id="quote_discount" class="form-control" style="width: 100%; padding: 10px; border: 1px solid var(--border); border-radius: 8px; background: var(--surface-3); color: var(--text-color);" placeholder="0.00" oninput="renderQuoteItems()" value="<?= $selected_quote['discount'] ?? 0 ?>">
            </div>
            <div style="flex: 1; min-width: 200px;">
                <label style="display: block; margin-bottom: 5px; font-weight: 600;">TOTAL FINAL ($)</label>
                <input type="number" step="0.01" name="quote_price" id="quote_price" class="form-control" style="width: 100%; padding: 10px; border: 1px solid var(--border); border-radius: 8px; background: var(--surface-3); font-weight: bold; color: var(--text-color); font-size: 1.1rem;" readonly value="<?= $selected_quote['total'] ?? 0 ?>">
            </div>
        </div>

        <!-- Action Buttons (Improved) -->
        <div style="margin-top: 30px; display: flex; gap: 15px; flex-wrap: wrap; justify-content: flex-end; padding-top: 20px; border-top: 1px solid var(--border);">
